...
class Scratch3TurtleBotBlocks {
    ...
    pubGo_straight (args) {
        ...
        const period_time = 0.032;//sec 速度更新周期 ☆0.03秒で速度を更新していない可能性あり!!→0.03secであっている！！
        var order_vale = 0;
        if(args.GO_STRAIGHT>0){
          order_vale = args.GO_STRAIGHT*0.01//m
        }
        else{order_vale = args.GO_STRAIGHT*-0.01}//m
        var moving_value = 0;//m
        var speed = 0;//m/sec
        var rate_of_increast = 0.01 ;//速度の増加率
        var moving_time_count = 0;//sec 各区間の移動時間を記録
        var sleep_time = 0;//sec 指定された距離を移動する時間
        var acc_section_remaining_vale = order_vale/5;//m 加速区間の移動距離
        console.log('Acc order value: '+ acc_section_remaining_vale + 'm');


        //加速区間の時間算出
        while(true){
          if (acc_section_remaining_vale>0){
            speed += rate_of_increast;//m/sec
            //console.log('Acc speed: '+ speed + 'm/sec');
            moving_value = speed * period_time;
            //console.log('Distance moved at current speed: '+ moving_value + 'm');
            moving_time_count += period_time;
            acc_section_remaining_vale -= moving_value;
          }else{break;}
        }
        console.log('After Acc order value: '+ acc_section_remaining_vale + 'm');
        sleep_time += moving_time_count;
        console.log('Acc travel time: '+ sleep_time + 'sec');

        //等速区間の時間算出
        const con_section_remaining_vale = order_vale*(3/5);//m 等速区間の移動距離
        sleep_time += con_section_remaining_vale / speed

        //変数初期化
        moving_time_count = 0;//sec 各区間の移動時間を記録
        //減速区間の時間算出
        var constant_velocity_section_speed = speed;//m/sec 等速区間の速度を代入
        moved_distance = order_vale*(4/5);//m 加速と等速区間で移動した距離
        var deceleration_rate = 0;//減速率
        var convergence_dodge_parameter = 0;//速度0にするための動的パラメータ
        var integration_coefficient = 0.1;//速度0にするための静的パラメータ　注意；ros側の値と揃える

        while(true){
          if (moved_distance>order_vale){
            deceleration_rate = (Math.abs(order_vale)-moved_distance)/(Math.abs(order_vale)/5);//減速率の値は進んだ距離が増加すると小さくなる
            speed = constant_velocity_section_speed * deceleration_rate + convergence_dodge_parameter;//速度の値は小さくなる
            convergence_dodge_parameter += (constant_velocity_section_speed - speed) * integration_coefficient;//この値も進んだ距離が増加すると小さくなる
            moved_distance += speed * period_time;
            moving_time_count += period_time;
          }else{break;}
        }
        ...
        console.log('Travel time: '+ sleep_time + 'sec');
